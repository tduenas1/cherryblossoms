...
```{r}
colors <- c("red", "#ffd92f", "orange", "firebrick", "magenta", "purple", "blue", "cornflowerblue", "darkgreen", "lightgreen")

x = cleaned_data$Time
y = cleaned_data$Age

plot(x,y, col = colors[factor(cleaned_data$Year - 1972)])
abline(lm(y~x), col = "black")

for (curr_year in 1973:1982)
{
  sub_data = cleaned_data %>% filter(Year == curr_year)
  abline(lm(sub_data$Age~sub_data$Time), col = colors[curr_year - 1972])
}

legend("topright", inset = .05, title = "Years",legend = 1973:1982, fill = c(1:7))
...
mutate(Percentile = case_when(
                         centile < .10 ~ percentile[1],
        centile >= .10 & centile < .20 ~ percentile[2],
        centile >= .20 & centile < .30 ~ percentile[3],
        centile >= .30 & centile < .40 ~ percentile[4],
        centile >= .40 & centile < .50 ~ percentile[5],
        centile >= .50 & centile < .60 ~ percentile[6],
        centile >= .60 & centile < .70 ~ percentile[7],
        centile >= .70 & centile < .80 ~ percentile[8],
        centile >= .80 & centile < .90 ~ percentile[9],
        centile >= .90 & centile <= 1.0 ~ percentile[10]
      ))%>% 
  select(-centile)
...
# cleaned_data_Performance$AgeGroup <- factor(cleaned_data_Performance$AgeGroup,
#                      levels = ageGroup,
#                      ordered = FALSE)
# summary(cleaned_data_Performance$AgeGroup)
